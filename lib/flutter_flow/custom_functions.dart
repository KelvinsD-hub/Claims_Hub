...
// 1. Check if inputs are null or empty to prevent errors
  if (phoneNumber == null || phoneNumber.isEmpty) {
    return null;
  }

  // Use a local variable to handle the non-null string
  String currentName = clientName ?? "Customer";

  // 2. Remove all non-numeric characters
  String cleanedNumber = phoneNumber.replaceAll(RegExp(r'\D'), '');

  // 3. Handle the Nigerian leading '0'
  if (cleanedNumber.startsWith('0')) {
    cleanedNumber = cleanedNumber.substring(1);
  }

  // 4. Ensure the 234 prefix is present
  if (!cleanedNumber.startsWith('234')) {
    cleanedNumber = '234' + cleanedNumber;
  }

  // 5. Create and encode the message
  String message =
      "Hello $currentName, I am reaching out from Claims Assist regarding your flight claim.";
...
String? generateWhatsAppGhNGN(
  String? phoneNumber,
  String? clientName,
) {
// 1. Check if inputs are null or empty
  if (phoneNumber == null || phoneNumber.isEmpty) {
    return null;
  }
...
// 4. Handle numbers that start with 00 (international format like 00233...)
  if (cleanedNumber.startsWith('00')) {
    cleanedNumber = cleanedNumber.substring(2);
  }

// 5. If number starts with 0 → determine country
  if (cleanedNumber.startsWith('0')) {
    // Remove leading 0
    cleanedNumber = cleanedNumber.substring(1);

    // Ghana numbers are 9 digits after removing 0
    if (cleanedNumber.length == 9) {
      cleanedNumber = '233' + cleanedNumber;
    }
    // Nigeria numbers are 10 digits after removing 0
    else if (cleanedNumber.length == 10) {
      cleanedNumber = '234' + cleanedNumber;
    }
  }

// 6. If still not starting with 233 or 234, assume Ghana by default
  if (!cleanedNumber.startsWith('233') && !cleanedNumber.startsWith('234')) {
    cleanedNumber = '233' + cleanedNumber;
  }

// 7. Create and encode message
  String message =
      "Hello $currentName, I am reaching out from Claims Assist regarding your flight claim.";
  String encodedMessage = Uri.encodeComponent(message);

// 8. Return WhatsApp URL
  return "https://wa.me/$cleanedNumber?text=$encodedMessage";
}
...
String? generateWhatsAppLink(
  ...
  // 1. Clean Number Logic (Ghana/Nigeria focus)
  String cleanedNumber = phoneNumber.replaceAll(RegExp(r'\D'), '');
  ...
  if (cleanedNumber.startsWith('0')) {
    String numWithoutZero = cleanedNumber.substring(1);
    if (numWithoutZero.length == 9) {
      cleanedNumber = '233' + numWithoutZero;
    } else if (numWithoutZero.length == 10) {
      cleanedNumber = '234' + numWithoutZero;
    } else {
      cleanedNumber = numWithoutZero;
    }
  }
  ...
  String message =
      "Hello $currentName, I am reaching out from Claims Assist. Please use this secure link to complete your claim: $magicLink";
  String encodedMessage = Uri.encodeComponent(message);
...
String? dialedCountryCode(String? countryName) {
  if (countryName == null) {
    return null;
  }
  final countryInfoJson = {
...
String? signatureImage(String? base64String) {
  if (base64String == null || base64String.isEmpty) {
    return null;
  }

  String cleanedBase64 = base64String.replaceAll('\n', '').trim();

  return 'data:image/png;base64,' + cleanedBase64;
}

FFUploadedFile? base64ToFile(String? base64String) {
  if (base64String == null || base64String.isEmpty) {
    return null;
  }

  try {
    // 1. Clean the string (removes the 'data:image/png;base64,' if present)
    final String cleanBase64 = base64String.contains(',')
        ? base64String.split(',').last
        : base64String;

    // 2. Decode the string
    // We use 'final' instead of 'Uint8List' to avoid import errors
    final decodedBytes = base64Decode(cleanBase64.trim());

    // 3. Return as FFUploadedFile
    return FFUploadedFile(
      bytes: decodedBytes,
      name: 'signature.png',
    );
  } catch (e) {
    // Returns null if the string is not a valid base64 format
    return null;
  }
}
